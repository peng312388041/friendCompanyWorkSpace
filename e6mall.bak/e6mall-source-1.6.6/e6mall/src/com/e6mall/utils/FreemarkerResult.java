...
public class FreemarkerResult extends StrutsResultSupport {
	...
	public void doExecute(String location, ActionInvocation invocation)
		...
		this.staticTemplate = (String) conditionalParse(staticTemplate,
				invocation);
		...
		// 生成静态页面的的模板类
		Template staticTemplate = configuration.getTemplate(
		...
		if (!filepath.exists()) { // 如果目录不存在，则创建它
			filepath.mkdirs();
		}
		Writer out = new BufferedWriter(new OutputStreamWriter(
				new FileOutputStream(path + fileName), "UTF-8"));// 加了设置输出文件编码UTF-8
	...
	protected Configuration getConfiguration() throws TemplateException {
		Configuration cfg = freemarkerManager
				.getConfiguration(ServletActionContext.getServletContext());
		cfg.setDefaultEncoding("UTF-8");// 加了编码,但没有效果
		// return
